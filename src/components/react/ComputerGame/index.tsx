...
const STATUS_TEXT: Record<ComputerGameStatus, string> = {
	...
	complete: 'Game over.',
	error: 'The computer could not choose a move.',
};
...
export const ComputerGameBar = React.memo((props: ComputerGame) => {
	const { engineName, playerColor, status, stop } = props;

	return (
		<div className={`cs-trainer is-computer is-${status}`}>
			<div className="cs-trainer-row">
				<span className="cs-trainer-chip">
					Playing against {engineName} as{' '}
					{playerColor === 'white' ? 'White' : 'Black'}
				</span>
				<span className="cs-trainer-status">
					{status === 'thinking'
						? `${engineName} is thinking...`
						: status === 'error'
						? `${engineName} could not choose a move.`
						: STATUS_TEXT[status]}
				</span>
